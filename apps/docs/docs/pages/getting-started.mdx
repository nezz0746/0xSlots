...
await client.liquidate("0xSlot");     // permissionless — no bounty, the slot itself is the reward
```
...
import { useSlotsClient, useSlotAction } from "@0xslots/sdk/react";
...
function SlotView({ address }: { address: string }) {
  ...
  const { data: slot, isLoading } = useQuery({
    ...
    queryFn: () => client.slotState(address as `0x${string}`),
  });

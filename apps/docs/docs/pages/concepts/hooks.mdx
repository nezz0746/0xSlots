...
resolves, the feed you post to, the seven-day tenure nobody can snipe through.
...
| `beforeSell` | decide | the occupant tries to sell into an offer |
| `beforeSelfAssess` | decide | the occupant re-prices |
| `afterBuy` | record | a buy went through |
| `afterSell` | record | a sale went through |
...
A hook can take a cut of the tax through `feeBps` / `feeRecipient`, skimmed at
collection; the rest goes to the recipient. That is how a hook author gets paid
for building something on slots they do not own — declared up front and visible
before anyone commits.

## Many behaviours: compose

A slot points at **exactly one** hook. To want several — a minimum tenure *and* a
feed *and* a fee — you point at a **`CompositeHook`** that fans out to children.

Keeping the fan-out in userland is not tidiness. The core makes **one** capped
call, and that single cap bounds the whole subtree beneath it, so a badly built
composite harms only the slot that chose it.

```mermaid
flowchart LR
    Slot -->|one capped call| C["CompositeHook"]
    C --> H1["Minimum tenure"]
    C --> H2["Feed post"]
    C --> H3["Fee split"]
```
...
says "this hook *claims* to be a 7-day tenure"; the factory's attestation is the
upgrade to "and we vouch for it." Authority is always the snapshotted flags,
never the label.
</Callout>

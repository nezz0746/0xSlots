...
flowchart LR
    ...
    Hook -.->|"optional fee, from the tax"| Recipient
...
No hook can prevent liquidation, and none can stop an occupant leaving.
`liquidate()` and `release()` never consult one.
</Callout>
...
You might want a swappable ad hook on tenure terms that never change.
Conflating them would make that inexpressible — and would let a slot advertising
"the hook may change" quietly also reserve the right to change who is allowed to
take it.

## Next

- [Hooks](/concepts/hooks) — how a slot gets a purpose, and why it might refuse a sale

...
| Entity | Key fields |
|---|---|
| `slot` | `id`, `chainId`, `recipient`, `currency`, `occupant`, `isOccupied`, `price`, `deposit`, `taxPercentage`, `collectedTax`, `taxPaidTotal`, `totalCollected`, `hook`, `tenureId`, `createdAt` |
...
needs an RPC call per slot: `pendingHasTax` / `pendingTaxPercentage`,
`pendingHasHook` / `pendingHook`, and `pendingProposedAt`.
...
query GetSlotActivity($slot: String!) {
  boughtEvents(where: { slot: $slot }, orderBy: "timestamp", orderDirection: "desc") {
    items { buyer from price deposit paid viaSell timestamp }
  }
